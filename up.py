...
if os.path.exists(homePath+'/myblog') is False and os.path.exists(homePath+'/3xp10it.github.io') is False:
    ...
    os.system('''cd ~/myblog && git pull && kill -9 $(ps -a | egrep "jekyll" | egrep -o "\d+" | head -n 1);sleep 3 && cp index.html tmp.html && cp index_bak.html index.html && sh -c "rm -r _site/ & jekyll serve --incremental --watch && exit" && cp _site/index.html index2.html && cp tmp.html index.html && echo congratulations! commands execute ok to here!''')
    os.system('''cd ~/3xp10it.github.io && git pull && cp -r ../myblog/_site/* . && cp ../myblog/index2.html . && cp ../myblog/index.html . && git status && git add . && git commit -a -m "update" && git push -u origin master && echo congratulations! commands execute ok to here!''')
    os.system('''cd ~/myblog && git pull && git status && git add . && git commit -a -m "update backup" && git push -u origin master && echo all is well,and all is done!''')
