...
#如果删除了~/pic目录下的图片,则要运行python3 mysnippingtool -ua表示手动更新address.txt中的列表
...
def main():
    ...
    if os.path.exists(pic_path) is False:
        print("this is the first time you use me,or you have deleted ~/pic,I will mkdir ~/pic and git pull the github's pic.git,please put pngs to ~/pic when needed,and don't delet any png file in this folder")
        os.system("mkdir ~/pic && cd ~/pic && git init && git pull https://github.com/3xp10it/pic.git && git remote add origin https://github.com/3xp10it/pic.git && git status")
        return

    address_path=pic_path+"/address.txt"

    if len(sys.argv)==2 and sys.argv[1]=="-ua":
        all_png_list=get_all_file_name(pic_path)
    ...
    os.system("cd ~/pic && git add . && git status && git commit -a -m 'update' && git push -u origin master")
    ...
    for each in all_png_list:
        try:
            ...
            print("open ~/pic/address.txt wrong")
...
it will get "~/桌面/spider_wooyun" as a result,but the true result should be "~/pic/spider_wooyun"
so I use:
current_file_abspath=os.path.isdir("~/pic/"+each):
